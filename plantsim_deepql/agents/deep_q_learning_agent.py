...
class DeepQLearningAgent(QLearningAgent):
    ...
    def train(self):
        ...
        # thus states and actions are saved in dictionaries with the id as key
        action = None
        actions = {}
        states = {}
        while True:
            ...
            if action is not None:
                ...
                self.N_sa[s, a] += 1
                self.q_table[s, a] = self.q_table[s, a] + self.alpha(s, a) * (
                            r + self.gamma * np.max(self.q_table[s_new]) - self.q_table[s, a])
