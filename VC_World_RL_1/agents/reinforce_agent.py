...
class PolicyNetwork(nn.Module):
    ...
    def get_action(self, state):
        #state = torch.from_numpy(np.array(state)).float().unsqueeze(0)
        probs = self.forward(state)
        highest_prob_action = np.eandom.choice(self.num_actions, p = np.squeeze(probs.cpu().detach.numpy()))
        #action = np.random.choice(np.squeeze(probs.cpu().detach.numpy()))
...
class ReinforceAgent(Agent):
    ...
    def __init__(self, problem, Optimizer=torch.optim.Adam, gamma=0.99, file="policy.npy"):
        ...
        self.policy = PolicyNetwork(len(self.states[0]), len(self.actions), Optimizer=Optimizer, gamma=gamma,
                                    transform=transform)
