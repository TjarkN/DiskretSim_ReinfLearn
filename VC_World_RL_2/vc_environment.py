...
class Building:
    ...
    def exist_room(self, position):
        if position[0] >= 0 and position[0] < self.rooms.shape[0] and position[1] >= 0 and position[1] < \
                self.rooms.shape[1]:
            return True
        else:
            return False
...
class VC(Problem):
    def __init__(self, size, dirt_rate=0.6, seed_value=None, building=None, position=None, energy_spend=0,
    ...
    def copy(self):
        ...
        vc_copy = VC(self.building.size, dirt_rate=self.dirt_rate, seed_value=self.seed_value, building=building_copy,
                     position=position_copy, energy_spend=self.energy_spend, random_action_rate=self.random_action_rate)
        return vc_copy
